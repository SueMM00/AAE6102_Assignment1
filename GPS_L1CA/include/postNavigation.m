...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        CNo_values = cell2mat(arrayfun(@(x) x.CNo.VSMValue(currMeasNr), trackResults(activeChnList), 'UniformOutput', false));

        % Calculate receiver position using WLS
        [WLSxyzdt,navSolutions.el(activeChnList, currMeasNr), ...
               navSolutions.az(activeChnList, currMeasNr), ...
               navSolutions.DOP(:, currMeasNr), navSolutions.Amatrix{currMeasNr,1}] =...
                       WLSPos(satPositions, clkCorrRawP, CNo_values, settings);
        ...
        [x_ekf, P_ekf] = EKF(satPositions, clkCorrRawP, carrFreq, settings);
        ...
        navSolutions.WLSX(currMeasNr)  = WLSxyzdt(1);
        navSolutions.WLSY(currMeasNr)  = WLSxyzdt(2);
        navSolutions.WLSZ(currMeasNr)  = WLSxyzdt(3);    
        
        % Receiver position using EKF in ECEF
        navSolutions.EKFX(currMeasNr)  = x_ekf(1);
        navSolutions.EKFY(currMeasNr)  = x_ekf(2);
        navSolutions.EKFZ(currMeasNr)  = x_ekf(3);    
        % Receiver velocity using EKF in ECEF
        navSolutions.EKFVx(currMeasNr)  = x_ekf(4);
        navSolutions.EKFVy(currMeasNr)  = x_ekf(5);
        navSolutions.EKFVz(currMeasNr)  = x_ekf(6);   

        % Compute velocity after obtaining position
        % Time step in seconds (sampling interval)
        deltaT = settings.navSolPeriod / 1000;  % Convert milliseconds to seconds

        % Check if deltaT is valid
        if isnan(deltaT) || deltaT <= 0
            error('Error: Time step (deltaT) is NaN, zero, or negative. Check settings.navSolPeriod.');
        end

        if currMeasNr > 1

            % Compute velocity components using numerical differentiation
            navSolutions.Vx(currMeasNr) = (navSolutions.WLSX(currMeasNr) - navSolutions.WLSX(currMeasNr - 1)) / deltaT;
            navSolutions.Vy(currMeasNr) = (navSolutions.WLSY(currMeasNr) - navSolutions.WLSY(currMeasNr - 1)) / deltaT;
            navSolutions.Vz(currMeasNr) = (navSolutions.WLSZ(currMeasNr) - navSolutions.WLSZ(currMeasNr - 1)) / deltaT;
    
            % Compute speed (magnitude of velocity)
            navSolutions.Speed(currMeasNr) = sqrt(navSolutions.Vx(currMeasNr)^2 + ...
                                          navSolutions.Vy(currMeasNr)^2 + ...
                                          navSolutions.Vz(currMeasNr)^2);
        else
            % First epoch: No velocity computation (set to NaN or zero)
            navSolutions.Vx(currMeasNr) = NaN;
            navSolutions.Vy(currMeasNr) = NaN;
            navSolutions.Vz(currMeasNr) = NaN;
            navSolutions.Speed(currMeasNr) = NaN;
        end
...
%% Coordinate conversion ==================================================
        ...
        %=== Convert WLS results to geodetic coordinates ==============================
        [navSolutions.WLSlatitude(currMeasNr), ...
         navSolutions.WLSlongitude(currMeasNr), ...
         navSolutions.WLSheight(currMeasNr)] = cart2geo(...
                                            navSolutions.WLSX(currMeasNr), ...
                                            navSolutions.WLSY(currMeasNr), ...
                                            navSolutions.WLSZ(currMeasNr), ...
                                            5);
        
        %=== Convert EKF results to geodetic coordinates ==============================
        [navSolutions.EKFlatitude(currMeasNr), ...
         navSolutions.EKFlongitude(currMeasNr), ...
         navSolutions.EKFheight(currMeasNr)] = cart2geo(...
                                            navSolutions.EKFX(currMeasNr), ...
                                            navSolutions.EKFY(currMeasNr), ...
                                            navSolutions.EKFZ(currMeasNr), ...
                                            5);
